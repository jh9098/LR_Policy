...
// 주식정보 테마 전용 입력 UI (amber 톤 + URL 안내 + 리스트 에디터 tone 지정)
...
function StockThemeEditor({ guide, onChange }) {
  ...
  const handleSectorField = (index, field, value) => {
    updateGuide((draft) => {
      if (!draft.sectorHighlights[index]) draft.sectorHighlights[index] = createSectorHighlight();
  ...
  const handleCompanyField = (index, field, value) => {
    updateGuide((draft) => {
      if (!draft.companyAnalyses[index]) draft.companyAnalyses[index] = createCompanyAnalysis();
    ...
    <section className="space-y-5 rounded-2xl border border-amber-200 bg-white p-6 shadow-sm dark:border-amber-500/40 dark:bg-slate-900/40">
      <header className="space-y-2">
        ...
        <p className="text-xs text-amber-700/80 dark:text-amber-200/80">
          시장 요약, 섹터 하이라이트, 기업 분석과 워치리스트를 JSON으로 관리합니다. (미리보기에서 URL은 자동 링크 처리)
        </p>
      </header>
      ...
      <label className="flex flex-col gap-2 text-sm">
        ...
        <textarea
          ...
          placeholder="이번 카드의 핵심 메시지 요약 (URL 입력 가능)"
        />
      </label>
      ...
      <label className="flex flex-col gap-2 text-sm">
        ...
        <textarea
          ...
          className="min-h-[100px] rounded-lg border border-amber-200 bg-white px-3 py-2 text-sm shadow-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 dark:border-amber-500/40 dark:bg-slate-900 dark:text-slate-100"
          placeholder="지수/금리/환율/수급 등 핵심 요약 (URL 입력 가능)"
        />
      </label>
      ...
      <div className="space-y-4">
        ...
        {safeGuide.sectorHighlights.length === 0 ? (
          <p className="rounded-lg border border-dashed border-amber-300 px-4 py-4 text-center text-xs text-amber-700 dark:border-amber-500/50 dark:text-amber-200">
            주요 섹터와 대표 종목을 정리해 주세요.
          </p>
        ) : null}
        ...
        <div className="space-y-6">
          {safeGuide.sectorHighlights.map((sector, index) => (
            <div
              key={`sector-${index}`}
              className="space-y-4 rounded-2xl border border-amber-200 bg-amber-50/60 p-5 dark:border-amber-500/40 dark:bg-slate-900"
            >
              ...
              <label className="flex flex-col gap-2 text-sm">
                ...
                <textarea
                  ...
                  placeholder="수요/공급, 가격, 정책 이슈 등 (URL 입력 가능)"
                />
              </label>
              ...
              <SimpleListEditor
                ...
                addLabel="종목 추가"
                itemPlaceholder="예: NVDA / AAPL / 005930"
              />
            </div>
          ))}
        </div>
      ...
      <div className="space-y-4">
        ...
        {safeGuide.companyAnalyses.length === 0 ? (
          <p className="rounded-lg border border-dashed border-amber-300 px-4 py-4 text-center text-xs text-amber-700 dark:border-amber-500/50 dark:text-amber-200">
            핵심 기업의 투자 논지와 촉매/리스크를 정리해 주세요. (URL 입력 가능)
          </p>
        ) : null}
            ...
            <div key={`company-${index}`} className="space-y-4 rounded-2xl border border-amber-200 bg-white p-5 dark:border-amber-500/40 dark:bg-slate-900">
              ...
              <label className="flex flex-col gap-2 text-sm">
                ...
                <textarea
                  ...
                  className="min-h-[100px] rounded-lg border border-amber-200 bg-white px-3 py-2 text-sm shadow-sm transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-400 dark:border-amber-500/40 dark:bg-slate-900 dark:text-slate-100"
                  placeholder="핵심 논지를 한 단락으로 작성 (URL 입력 가능)"
                />
              </label>
              ...
              <SimpleListEditor
                ...
                addLabel="촉매 추가"
                itemPlaceholder="예: 신규 제품 출시, 원가 하락, 고객사 증설 기사 링크"
              />
              ...
              <SimpleListEditor
                ...
                addLabel="리스크 추가"
                itemPlaceholder="예: 규제 강화, 수요 둔화, 고객 이탈, 악재 기사 링크"
              />
      ...
      <SimpleListEditor
        ...
        addLabel="항목 추가"
        itemPlaceholder="예: 달러인덱스, 유가, 특정 종목 티커/뉴스 링크"
      />
    </section>
  );
}
...
StockThemeEditor.defaultProps = { guide: null };
