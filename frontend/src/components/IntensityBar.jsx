...
// 주장 강도(intensity) 수치를 시각화하는 컴포넌트다.
// Firestore 데이터 구조와 직접 연결되며, 값은 0~100 범위의 정수라고 가정한다.
...
const AVAILABLE_CLASSES = ['bg-indigo-500', 'bg-emerald-500', 'bg-rose-500', 'bg-amber-500', 'bg-sky-500', 'bg-violet-500'];
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="h-2 w-full rounded-full bg-slate-200 dark:bg-slate-700" aria-hidden="true">
        <div className={`h-2 rounded-full transition-all duration-300 ease-out ${safeColor}`} style={{ width: `${safeValue}%` }} />
      </div>
