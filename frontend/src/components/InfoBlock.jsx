...
function InfoBlock({ title, badgeType, headline, body, note }) {
  const badge = BADGE_STYLES[badgeType] ?? BADGE_STYLES.fact;

  return (
    <section className="relative overflow-hidden rounded-lg border border-slate-200 bg-white p-6 shadow-sm transition hover:shadow-md">
      <span
      ...
      <div className="mt-12 space-y-4">
        <header className="space-y-1">
          ...
          {headline && <p className="text-sm font-medium text-slate-600">{headline}</p>}
        </header>
        {renderBody(body)}
