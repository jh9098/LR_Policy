...
// 홈 화면에서 Firestore에 직접 연결해 최근 이슈를 불러오고 필터링한다.
// TODO: 현재는 누구나 읽기가 가능하며 Render 백엔드를 전혀 사용하지 않는다.
...
function HomePage() {
  ...
  const loadRecentIssues = useCallback(async () => {
    ...
    try {
      ...
      const sorted = sortIssuesByDate(data);
      setIssues(sorted);
  ...
  const handleSearch = async () => {
    ...
    setError('');
    try {
      const data = await searchIssuesClient({ queryText: queryDraft, category: categoryDraft, maxCount: 80 });
      const sorted = sortIssuesByDate(data);
      setIssues(sorted);
    ...
    <section className="space-y-8">
      ...
      <header className="rounded-2xl border border-slate-200 bg-white px-5 py-6 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:px-6 sm:py-8">
        ...
        <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-slate-600 dark:text-slate-300">
          핵심 내용을 먼저 이해하고, 마지막에 쟁점과 시각 차이를 확인하세요. 이 서비스는 사실과 맥락을 기반으로 정보를 정리한 후, 선택적으로 진영별 주장을 덧붙입니다.
        </p>
      </header>
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800 sm:p-6">
        ...
        <p className="mt-1 text-[13px] leading-relaxed text-slate-600 dark:text-slate-300">
          카테고리와 키워드로 궁금한 정책/사건을 찾아보세요. 검색 버튼을 누르면 Firestore에서 불러온 목록을 프론트에서 다시 필터링합니다.
        </p>
        ...
        <p className="mt-3 text-[11px] leading-relaxed text-slate-500 dark:text-slate-400">
          ※ 현재는 Firestore에서 최대 80건을 불러온 뒤 메모리에서 필터링합니다. 데이터가 많아지면 별도의 검색 인덱스 도입이 필요합니다.
        </p>
      </section>
