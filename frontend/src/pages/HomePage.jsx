...
function HomePage() {
  ...
  const [isScrapOpen, setScrapOpen] = useState(() => scrapParam === 'open');
  ...
  }, [user, loadScraps, scrapParam, setSearchParams]);
      ...
      {user && isScrapOpen ? (
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
            <div>
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">내 스크랩</p>
              <p className="text-sm text-slate-500 dark:text-slate-400">{displayName}님의 저장한 게시물을 모았습니다.</p>
            </div>
            <button
              type="button"
              onClick={handleCloseScrap}
              className="inline-flex items-center justify-center rounded-full border border-slate-300 px-3 py-1 text-xs font-semibold text-slate-600 transition hover:border-slate-400 hover:text-slate-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 focus-visible:ring-offset-2 focus-visible:ring-offset-white dark:border-slate-600 dark:text-slate-300 dark:hover:border-slate-500 dark:hover:text-slate-100 dark:focus-visible:ring-offset-slate-900"
            >
              닫기
            </button>
          </div>
